    ...
    for i in 0..clause.literals.len() {
        for j in (i + 1)..clause.literals.len() {
            ...
            if unifier.unify(&lit1.atom, &lit2.atom).is_ok() {
                ...
                factored_clause.proof_depth = clause.proof_depth;
                // Preserve existing parents from the input clause
                for parent in clause.parents.iter() {
                    factored_clause.add_parent(*parent);
                }
                // Optionally add the clause's own ID if provided
