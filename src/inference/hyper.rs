...
fn hyperresolve_recursive(
    ...
    if current_neg_idx >= neg_indices.len() {
        ...
        result_clause.proof_depth = current_max_depth + 1;
        ...
        // Debug: catch hyperresolution producing empty parents
        if result_clause.parents.is_empty() {
            eprintln!("HYPER BUG: Produced clause with empty parents!");
            eprintln!("  Nucleus ID: {:?}", nucleus_id);
            eprintln!("  Satellite IDs: {:?}", used_satellites);
            eprintln!("  Result: {:?}", result_clause);
        }
    ...
    for (sat_idx, satellite) in satellites.iter().enumerate() {
        ...
        for (lit_idx, sat_lit) in satellite.literals.iter().enumerate() {
            ...
            for (_var, _term) in current_subst.iter() {
                // Substitution handling would go here
            }

            // Try unification
            let combined_subst = {
                let test_subst = current_subst.clone();
            ...
            let _new_pos_literals = pos_literals.to_vec();
            for (i, lit) in satellite.literals.iter().enumerate() {
                if i != lit_idx && lit.sign {
                    // Add renamed literal
                    let _renamed = rename_literal(lit, var_offset);
            ...
            hyperresolve_recursive(
                ...
                new_used,
                var_offset + 100,
                std::cmp::max(current_max_depth, satellite.proof_depth),
                results,
            );
        }
    }
...
pub fn hyperresolve_units(
    ...
    // Hyperresolution starting (debug output removed for performance)
...
fn hyperresolve_units_recursive(
    ...
    // Resolving negative literal (debug output removed for performance)
    ...
    for (orig_idx, satellite) in unit_satellites {
        ...
        // Successful unification (debug output removed for performance)
