...
//!
//! Based on the original Foras 3.3 linkur.c implementation.

use crate::data::{Clause, ClauseId, Literal};
use crate::inference::resolution::rename_variables;
...
pub struct LinkedURConfig {
    ...
    /// Maximum iterations in main search loop
...
pub struct LinkNode {
    ...
    _unit_deleted: bool,                 // Unit deletion applied?
    goal: Literal,                      // Literal to resolve
    _goal_to_resolve: Option<Literal>,   // Instantiated form after substitution
...
///
/// FIXED: Converted from recursive to iterative to prevent stack overflow
pub fn backward(
    current: Rc<RefCell<LinkNode>>,
    ...
    let max_backtrack_steps = 10000; // Safety limit to prevent infinite loops
    let mut step_count = 0;

    loop {
        step_count += 1;
        if step_count > max_backtrack_steps {
            eprintln!("WARNING: Linked UR backtracking exceeded max steps, aborting");
            return None;
        }
        ...
        // Try next sibling first
        {
            ...
            if let Some(ref next_sib) = node_borrowed.next_sibling {
                return Some(next_sib.clone());
            }
        }
        ...
        // Continue backtracking from parent
        node = parent;
    }
}
...
pub fn generate_resolvent(
    ...
    for clause in usable {
        ...
        let renamed_clause = rename_variables(clause, var_offset);

        // Try to unify with each literal in the renamed clause
        for (idx, literal) in renamed_clause.literals.iter().enumerate() {
            ...
            // literal already has renamed variables, but we must apply current_subst too?
            // Wait. current_subst maps variables from previous steps (nucleus + prev satellites).
            // renamed_clause has NEW variables (offset).
            // current_subst does NOT contain these new variables yet.
            // So applying it to literal is effectively identity for the new variables.
            // But just in case current_subst has something weird, we apply it.
            ...
            if unifier.unify(&goal_atom, &literal_atom).is_ok() {
                ...
                return Some(ResolventResult {
                    clause: renamed_clause, // Return the renamed clause
...
fn build_ur_resolvent(
    ...
    let mut max_depth = nucleus.proof_depth;
    
    // Check all resolved literals in the tree for their proof depth
    let tree_borrowed = tree.borrow();
    let mut current_child = tree_borrowed.first_child.clone();
    
    while let Some(child_node) = current_child {
        let child = child_node.borrow();
        if let Some(ref clause) = child.current_clause {
            max_depth = std::cmp::max(max_depth, clause.proof_depth);
        }
        current_child = child.next_sibling.clone();
    }

    result.proof_depth = max_depth + 1;
...
pub fn linked_ur_resolve(
    ...
    loop {
        ...
        if iteration_count > config.max_iterations {
            eprintln!("WARNING: Linked UR exceeded max iterations ({}), stopping search", config.max_iterations);
            break; // Safety limit
        }
        ...
        // Calculate variable offset based on depth to ensure unique variables
        // Use a large multiplier to avoid collisions (e.g. 1000 per level)
        // Level 0 is nucleus. Level 1 is first satellite (1000), etc.
        let var_offset = ((depth as u16) + 1) * 1000;

        // Debug: trace for Nucleus 4 (CD)
        let debug = nucleus_id == Some(crate::data::ClauseId(4));
        if debug {
            eprintln!("LUR: Depth {}, Goal: {:?}, Subst: {:?}", depth, goal, current_subst);
        }

        // Try to find a clause that resolves with this goal
        match generate_resolvent(&goal, usable, &current_subst, var_offset) {
            Some(resolvent) => {
                // Found a unifiable clause!
                if debug {
                    eprintln!("LUR: Resolved with clause: {:?}, New Subst: {:?}", resolvent.clause, resolvent.substitution);
                }
                ...
                if is_fully_resolved {
                    ...
                    let mut final_clause = build_ur_resolvent(&root, nucleus, &resolvent.substitution);

                    let parent_ids = vec![nucleus_id];
                    // In full implementation, collect all satellite clause IDs
                    
                    if let Some(id) = nucleus_id {
                        final_clause.add_parent(id);
                    }
                    ...
                    match forward(current.clone(), None) {
                        Some(next) => {
                            // Propagate substitution to the next goal
                            next.borrow_mut().subst = resolvent.substitution.clone();
                            current = next;
                            depth = current.borrow().depth();
                        }
                        None => break,
...
mod tests {
    use super::*;
    use crate::data::{SymbolTable, SymbolKind, Term};
    ...
    fn test_generate_resolvent_simple() {
        ...
        let result = generate_resolvent(&goal, &[clause], &subst, 100);
