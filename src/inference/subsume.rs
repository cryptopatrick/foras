...
use crate::data::{Clause, Literal, Term};
...
/// Returns false if recursion depth exceeds limit (prevents stack overflow).
fn match_term(general: &Term, specific: &Term, subst: &mut Substitution) -> bool {
    match_term_with_depth(general, specific, subst, 0)
}

/// One-way matching with depth limit to prevent stack overflow.
const MAX_TERM_DEPTH: usize = 1000;

fn match_term_with_depth(
    general: &Term,
    specific: &Term,
    subst: &mut Substitution,
    depth: usize,
) -> bool {
    // Prevent stack overflow from deeply nested terms
    ...
    match (general, specific) {
        ...
        (Term::Variable { id, .. }, _) => {
            if let Some(bound) = subst.lookup(*id) {
                ...
                terms_equal_with_depth(bound, specific, 0)
            } else {
        ...
        (
            ...
            for (a1, a2) in args1.iter().zip(args2.iter()) {
                if !match_term_with_depth(a1, a2, subst, depth + 1) {
                    return false;
                }
            }
            true
...
/// Check if two terms are structurally equal with depth limit.
fn terms_equal_with_depth(t1: &Term, t2: &Term, depth: usize) -> bool {
    ...
    match (t1, t2) {
        ...
        (
            ...
            s1 == s2
                && args1.len() == args2.len()
                && args1.iter().zip(args2.iter()).all(|(a, b)| terms_equal_with_depth(a, b, depth + 1))
        }
        _ => false,
    }
}
...
/// Check if a clause is subsumed by any clause in a list, using ancestor subsumption.
///
/// Ancestor subsumption: if clause C subsumes D and D subsumes C (bidirectional),
/// they are equivalent. In this case, prefer the one with fewer parents (shorter proof).
///
/// Returns true if the clause should be discarded (subsumed).
pub fn forward_subsumed_ancestor(clause: &Clause, clauses: &[&Clause]) -> bool {
    for existing in clauses {
        if subsumes(existing, clause) {
            // existing subsumes clause
            // Check if clause also subsumes existing (bidirectional)
            if subsumes(clause, existing) {
                // They subsume each other (equivalent clauses)
                // Prefer the one with shorter proof derivation (depth)
                // Discard clause ONLY if existing has shorter or equal proof depth
                if existing.proof_depth <= clause.proof_depth {
                    return true; // Discard clause (existing is better or equal)
                }
                // Otherwise clause has fewer parents (better), keep it
                // Don't return true, but also need to mark existing for removal
                // For now, just keep searching - clause won't be subsumed by this one
                continue;
            } else {
                // Only existing subsumes clause (not bidirectional)
                // Regular subsumption: discard clause
                return true;
            }
        }
    }
