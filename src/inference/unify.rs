...
const MAX_UNIFY_DEPTH: usize = 500;
...
impl Substitution {
    ...
    pub fn apply(&self, term: &Term) -> Term {
        self.apply_with_depth(term, 0)
    }

    fn apply_with_depth(&self, term: &Term, depth: usize) -> Term {
        ...
        match term {
            Term::Variable { id, symbol } => {
                if let Some(bound_term) = self.lookup(*id) {
                    ...
                    self.apply_with_depth(bound_term, depth + 1)
                } else {
            ...
            Term::Application { symbol, args } => {
                let new_args = args
                    .iter()
                    .map(|arg| self.apply_with_depth(arg, depth + 1))
                    .collect();
...
impl Unifier {
    ...
    fn unify_with_depth(
        ...
        match (&t1, &t2) {
            ...
            (Term::Variable { id, .. }, _) => {
                if self.occurs_in_with_depth(*id, &t2, depth) {
            ...
            (_, Term::Variable { id, .. }) => {
                if self.occurs_in_with_depth(*id, &t1, depth) {
                    return Err(UnificationError::OccursCheck {
                        var: *id,
                        term: t1,
                    });
            ...
            (
                ...
                for (a1, a2) in args1.iter().zip(args2.iter()) {
                    self.unify_with_depth(a1, a2, depth + 1)?;
                }
    ...
    fn occurs_in(&self, var: VariableId, term: &Term) -> bool {
        self.occurs_in_with_depth(var, term, 0)
    }

    fn occurs_in_with_depth(&self, var: VariableId, term: &Term, depth: usize) -> bool {
        // Prevent stack overflow
        if depth > MAX_UNIFY_DEPTH {
            return false;
        }
        ...
        match term {
            Term::Variable { id, .. } => {
                ...
                if let Some(bound) = self.substitution.lookup(*id) {
                    return self.occurs_in_with_depth(var, bound, depth + 1);
                }
                false
            }
            Term::Application { args, .. } => {
                args.iter().any(|arg| self.occurs_in_with_depth(var, arg, depth + 1))
            }
        }
    }
}
