...
fn try_ur_resolve_keeping(
    ...
    let mut max_parent_depth = nucleus.proof_depth;
    ...
    for (_, nucleus_lit) in resolve_literals {
        ...
        for (sat_idx, satellite) in unit_satellites {
                ...
                .is_ok()
            {
                ...
                max_parent_depth = std::cmp::max(max_parent_depth, satellite.proof_depth);
                found = true;
                break;
            }
        }
    ...
    let mut clause = Clause::new(vec![result_literal]);
    clause.proof_depth = max_parent_depth + 1;

    if let Some(id) = nucleus_id {
        clause.add_parent(id);
    }
    for sat_id in &result_satellite_ids {
...
mod tests {
    use super::*;
    use crate::data::{SymbolKind, SymbolTable, Term};
