...
/// Different kinds of list sections encountered in Foras input.
...
pub struct ForasFile {
    pub lists: Vec<ListSection>,
    pub commands: Vec<ForasCommand>,
    pub operators: crate::parser::OperatorTable,
}

impl Default for ForasFile {
    fn default() -> Self {
        Self {
            lists: Vec::new(),
            commands: Vec::new(),
...
impl Parser {
    ...
    pub fn parse_str(&self, source: &str) -> Result<ForasFile, ParseError> {
        let mut file = ForasFile::default();
        ...
        for (line_idx, raw_line) in source.lines().enumerate() {
            ...
            if let Some((ref mut buffer, ref mut depth)) = proof_buffer {
                ...
                if *depth == 0 {
                    process_command(&mut file, ForasCommand::ProofObject(buffer.clone()));
                    proof_buffer = None;
                }
                continue;
            }
            ...
            if line.starts_with("((") {
                ...
                if depth == 0 {
                    ...
                    process_command(&mut file, ForasCommand::ProofObject(line.to_string()));
                } else {
...
/// Check if a name is a variable in Foras's default mode.
/// In Foras, variables are names starting with 'u', 'v', 'w', 'x', 'y', or 'z'.
/// Examples: x, y, z, x1, y2, u, v, w are all variables.
/// Constants start with other letters (a-t) or are multi-char names like "abc".
fn is_foras_variable(name: &str) -> bool {
    if name.is_empty() {
        return false;
    }
    let first_char = name.chars().next().unwrap();
    // In Foras default mode, variables start with u-z
...
fn compute_variable_id(name: &str) -> u16 {
    if name.len() == 1 {
        ...
        match c {
            ...
            'v' => 4,
            'w' => 5,
            _ => 0, // Shouldn't happen if is_foras_variable is true
        }
    } else {
...
fn parse_term(
    ...
    // In Foras (default mode), variables are names starting with u-z.
    // E.g., x, y, z, x1, y2, u, v, w are all variables.
    // In Prolog style (set(prolog_style_variables)), variables start with A-Z or _.
    // Here we implement the default Foras convention.
    if is_foras_variable(text) {
...
fn find_infix_operator<'a>(
    ...
    // Higher precedence binds less tightly in Foras, so we check them first
...
fn process_command(file: &mut ForasFile, command: ForasCommand) {
    // If it's an op() command, update the operator table
    if let ForasCommand::Op { precedence, ref fixity, ref symbol } = command {
    ...
    if let ForasCommand::MakeEvaluable { ref operator, .. } = command {
...
fn parse_command(text: &str) -> ForasCommand {
    ...
    if lower.starts_with("lex(") && text.ends_with(')') {
        ...
        if inner.starts_with('[') && inner.ends_with(']') {
            ...
            let symbols: Vec<String> = split_arguments(list_content)
                ...
                .filter(|s| !s.is_empty())
                .collect();
            return ForasCommand::Lex { symbols };
        }
    }
    if lower.starts_with("make_evaluable(") && text.ends_with(')') {
        ...
        if let Some(comma) = find_top_level_char(inner, ',') {
            ...
            return ForasCommand::MakeEvaluable {
                operator,
                evaluator,
            };
        }
    }
    ForasCommand::Generic(text.to_string())
}
...
#[cfg(test)]
mod tests {
    use super::{ListKind, ForasCommand, Parser};
    ...
    fn parse_simple_list() {
        ...
        assert!(matches!(file.commands[0], ForasCommand::Set(_)));
    ...
    fn op_command_is_parsed() {
        ...
        match &file.commands[0] {
            ForasCommand::Op { precedence, fixity, symbol } => {
